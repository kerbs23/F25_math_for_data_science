...
I used Nevstrov gradient decent, with a momentum of .9 and a step size of .001.
...
Note: 95\% student's t for n=8 is 2.3.
...
The big network is 6 layers of linear/relu, I just tried to make the simple one twice as "long". It does not preform significantly different from the simple network, but was stopped much faster.
This suggests to me the stopping rule is too strict for this large a model, but the image layer's consistently better performance suggests otherwise.
"Further research is required" type moment.

The image network is like the simple network, but with 2 alternating sections of convolution (learnable interaction between adjacent pixels) and maxpool (deterministic dim reduction) added to the front. Took much longer to converge, but did barely statisticaly better then the other models.
